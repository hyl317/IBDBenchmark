...
do
    ...
    do
        if [[ "$id1" == "$id2" || "$id1" > "$id2" ]]; then
            ...
            cd "${id1}_${id2}"
            # bcftools merge -O v -o "${id1}_${id2}.tmp.vcf" $base/$id1/$id1.minDP$DP.all.vcf.gz $base/$id2/$id2.minDP$DP.all.vcf.gz
            # bgzip "${id1}_${id2}.tmp.vcf"
            # bcftools view -O v -o "${id1}_${id2}.vcf" -g ^miss "${id1}_${id2}.tmp.vcf.gz"
            bcftools merge -O u $base/$id1/$id1.minDP$DP.all.vcf.gz $base/$id2/$id2.minDP$DP.all.vcf.gz | bcftools view -O z -o "${id1}_${id2}.vcf.gz" -g ^miss -i 'MAC>=1'
            # rm "${id1}_${id2}.tmp.vcf.gz"
            ...
            /mnt/archgen/users/yilei/bin/ibis/ibis -bfile "${id1}_${id2}" -noFamID -o "${id1}_${id2}" -mL 3
