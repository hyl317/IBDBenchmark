...
if __name__ == '__main__':
    ...
    # second, add errors 
    in_path = f"../realMosaicSim_wgs/simGeno/ch3_{args.l}cm/sim_ch3.h5" # simulated genotypes of modern synthetic individuals
    shuffle_rates = [0.5, 0.302, 0.213, 0.180, 0.143, 0.099, 0.069]
    for err, shuffle_rate in zip(['cov3', 'cov2', 'cov1', 'cov3over4', 'cov1over2', 'cov1over4', 'cov1over10'], shuffle_rates):
        if err != 'cov3':
            break
        ...
        hapBLOCK_chroms(folder_in=f"./simGeno/ch3_{args.l}cm/sim_{err}_ch", iids = iids, run_iids = run_iids, 
                ch=3, folder_out=f'./calledIBD/{err}/ch3_{args.l}cm/', output=False, prefix_out='', logfile=False,
