...
do
    ...
    #             echo "$id1.batch$b.bam    $id1" > reheader
    #             echo "$id2.batch$b.bam    $id2" >> reheader

    #             bcftools merge $base/$id1/$cov/batch$b/$id1.batch$b.phased.filter1.vcf.gz \
    #             $base/$id2/$cov/batch$b/$id2.batch$b.phased.filter1.vcf.gz | bcftools reheader -s reheader | bcftools view -s $id1,$id2 -O z -o "$id1"_"$id2".batch$b.merged.wgs.vcf.gz -e 'FORMAT/GT[*]="./."'

    #             bcftools index "$id1"_"$id2".batch$b.merged.wgs.vcf.gz
    #             rm reheader

    #             plink --vcf "$id1"_"$id2".batch$b.merged.wgs.vcf.gz --make-bed --out "$id1"_"$id2".batch$b
    #             /mnt/archgen/users/yilei/bin/ibis/add-map-plink.pl "$id1"_"$id2".batch$b.bim /mnt/archgen/users/yilei/Data/Hapmap/genetic_map_GRCh37_chr{1..22}.txt > new.bim
    #             rm "$id1"_"$id2".batch$b.bim
    #             mv new.bim "$id1"_"$id2".batch$b.bim

    #             # run IBIS
    #             /mnt/archgen/users/yilei/bin/ibis/ibis -bfile "$id1"_"$id2".batch$b -noFamID -o batch$b -min_l 5
    #             cd ../
    #         fi
    #     done
    # done
