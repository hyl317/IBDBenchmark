...
ch=3
i=$SGE_TASK_ID
id="$(sed "${i}q;d" iid.list)"

mkdir -p $id/impute_trim5/ch3/
cd $id/impute_trim5/ch3

# path2BAM="/mnt/archgen/users/yilei/IBDsim/BAM/$id.bam"
# trimmedBAM="$id.trim5.ch3.bam"
# samtools view -b -o $id.ch3.bam $path2BAM 3
# /mnt/archgen/users/yilei/bin/bamUtil-master/bin/bam trimBam $id.ch3.bam $trimmedBAM -L 5 -R 5
# samtools index $trimmedBAM
...
# bcftools mpileup -f ${REFGEN} -I -B -a 'FORMAT/DP' -r $ch -T ${VCF} -q 30 -Q 30 ${trimmedBAM} -Ou | bcftools call -Aim -C alleles -T ${TSV} -Oz -o ${OUT}
# bcftools index -f ${OUT}

# ####### delete trimmed bam file to save storage space
# rm $id.ch3.bam
# rm $trimmedBAM
# rm $trimmedBAM.bai
# ###################################################
...
bcftools view -Oz -o $id.ch$ch.GP99.MAF5.vcf.gz -i 'MAX(FORMAT/GP)>=0.99&&INFO/RAF>=0.05&&INFO/RAF<=0.95' $id.chr$ch.merged.vcf.gz
bcftools index $id.ch$ch.GP99.MAF5.vcf.gz